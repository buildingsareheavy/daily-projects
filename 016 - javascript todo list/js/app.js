...
// When you clikc on something with #add, do the following:
// Something strikes me as wrong about naming a button with an id, especially if it repeats
document.getElementById("add").addEventListener("click", function() {
  var value = document.getElementById("item").value;
  if (value) {
    addItemTodo(value);
    // Everytime you add an item, it will reset to a blank value
    // Could you / should you use null?
    document.getElementById("item").value = "";
  }
});

function removeItem() {
  // Still confused about the parentNode thing. I get that it's a value, but like, huh?
  // Right around 50 mins or so, maybe after explains it.
...
function addItemTodo(text) {
  ...
  // I kept getting confused that this variable was called "remove". I thought it was a built-in function or something
  ...
  // AppendChild means add it to the DOM
  buttons.appendChild(remove);
  buttons.appendChild(complete);
  item.appendChild(buttons);
  // Add item before all other items
